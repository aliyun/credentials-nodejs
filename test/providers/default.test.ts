...
describe('DefaultCredentialsProvider', function () {
  ...
  it('getCredentials', async function () {
    ...
    let provider = DefaultCredentialsProvider.builder().build();
    assert.ok((provider as any).providers.length === 1);
    try {
      await provider.getCredentials();
      assert.fail();
    } catch (ex) {
      assert.strictEqual(ex.message, 'unable to get credentials from any of the providers in the chain: unable to get credentials from enviroment variables, Access key ID must be specified via environment variable (ALIBABA_CLOUD_ACCESS_KEY_ID)');
    }
